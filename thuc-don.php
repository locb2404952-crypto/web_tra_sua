...
// ========================================================
// XỬ LÝ KHI KHÁCH HÀNG BẤM NÚT "XÁC NHẬN ĐẶT HÀNG" (NHẬN MẢNG CART JSON)
// ========================================================
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn_dat_hang'])) {
    ...
    // BỔ SUNG: Nhận địa chỉ từ Form gửi lên
    $dia_chi = mysqli_real_escape_string($conn, $_POST['khach_dia_chi']); 
    
    // Nhận chuỗi JSON chứa toàn bộ danh sách món từ client gửi lên
    $cart_data_json = $_POST['cart_data'];
    $cart_items = json_decode($cart_data_json, true);

    if (empty($ten_khach) || empty($sdt) || empty($dia_chi) || empty($cart_items)) {
        $thong_bao = "vui_long_nhap_du";
    } else {
        // 1. Tính toán tổng tiền thực tế của đơn hàng
        $total_order_amount = 0;
        foreach ($cart_items as $item) {
            $total_order_amount += floatval($item['price']) * intval($item['quantity']);
        }

        // Lấy user_id nếu đã đăng nhập, mặc định = 2 nếu là khách vãng lai
        $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 2;

        // BỔ SUNG LƯU ĐỊA CHỈ: Tạm thời tận dụng nối chung thông tin địa chỉ/SĐT hoặc nếu database có cột note/address thì lưu vào. 
        // Để không làm lỗi cấu trúc bảng hiện tại của nhóm, ta sẽ lưu địa chỉ chung vào đơn hàng hoặc một ghi chú tổng quan (hoặc nếu bảng orders của nhóm có cột địa chỉ thì tuyệt vời).
        ...
        if (mysqli_query($conn, $sql_order)) {
            ...
            // Bước B: Vòng lặp bóc tách mảng, chèn từng dòng sản phẩm kèm tùy chọn vào order_items
            foreach ($cart_items as $item) {
                ...
                // NỐI ĐỊA CHỈ VÀO PHẦN GHI CHÚ ĐỂ SHIPPER DỄ NHÌN THẤY (Hoặc ghi chú món ăn)
                $topping_note = mysqli_real_escape_string($conn, $item['topping']);
                $full_note = "Dia chi: " . $dia_chi . " | Note: " . $topping_note;

                $sql_item = "INSERT INTO order_items (order_id, product_id, quantity, price, sugar_level, ice_level, topping_note) 
                             VALUES ($order_id, $product_id, $quantity, $price, $sugar_level, $ice_level, '$full_note')";
...
// LẤY DANH SÁCH DANH MỤC SẢN PHẨM RA GIAO DIỆN
...
<head>
    ...
    <title>Thực Đơn Gọi Món - Trà Sữa Homie</title>
    ...
    <style>
        ...
        .back-home-btn { position: absolute; top: 25px; left: 30px; color: white; text-decoration: none; font-weight: bold; font-size: 15px; background: rgba(255,255,255,0.2); padding: 6px 15px; border-radius: 20px; }
        .back-home-btn:hover { background: white; color: var(--primary-color); }
        ...
        .product-image { width: 100%; height: 160px; background-color: #ffeaa7; display: flex; align-items: center; justify-content: center; font-size: 55px; user-select: none; }
        .product-info { padding: 20px; display: flex; flex-direction: column; flex-grow: 1; }
        .product-name { font-size: 19px; font-weight: bold; margin: 0 0 8px 0; }
        ...
        .btn-select { background-color: var(--primary-color); color: white; border: none; padding: 9px 20px; border-radius: 25px; font-weight: bold; cursor: pointer; box-shadow: 0 3px 8px rgba(255, 118, 117, 0.3); }
        .btn-mini-cart { background-color: #ffeaa7; color: #d63031; border: none; width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 16px; cursor: pointer; }
        ...
        .modal-content { background-color: white; padding: 25px; border-radius: 14px; width: 440px; max-width: 90%; position: relative; animation: formFadeIn 0.3s ease; }
        ...
        .form-control { width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; }
        ...
        .quantity-counter { display: flex; align-items: center; gap: 12px; }
        ...
        .btn-delete-item { color: #e74c3c; background: none; border: none; cursor: pointer; }
...
<header>
    <a href="index.php" class="back-home-btn"><i class="fa-solid fa-arrow-left"></i> Về Trang Chủ</a>
    <h1>🍽️ Thực Đơn Gọi Món 🍽️</h1>
</header>
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-price-action">
                            ...
                            <div class="action-buttons-group">
                                ...
                                <button class="btn-mini-cart" title="Thêm nhanh" onclick="themNhanhVaoGioHang(<?= $prod['product_id'] ?>, '<?= htmlspecialchars($prod['product_name']) ?>', <?= $prod['price'] ?>, <?= $cat_id ?>)">
...
<div id="cartModal" class="modal">
    <div class="modal-content" style="width: 500px;">
        ...
        <div id="cartItemsList" style="max-height: 200px; overflow-y: auto; margin-bottom: 20px; border-bottom: 2px solid #f1f2f6; padding-bottom: 10px;"></div>
        ...
        <form action="" method="POST" onsubmit="return validateCartBeforeSubmit()">
            ...
            <div class="form-group">
                ...
                <input type="text" name="khach_dia_chi" class="form-control" required placeholder="Số nhà, tên đường, số phòng...">
            </div>
...
<div id="floating-cart" onclick="moGioHangHienTai()" style="position: fixed; bottom: 30px; right: 30px; width: 60px; height: 60px; background-color: #ff7675; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; color: white; cursor: pointer; box-shadow: 0 4px 15px rgba(255, 118, 117, 0.4); z-index: 999;">
...
function themNhanhVaoGioHang(productId, productName, price, categoryId) {
    ...
    showToast("✨ Đã thêm nhanh '" + productName + "' thành công!");
}
...
function moTuyChonMon(productId, productName, price, categoryId) {
    ...
    if (categoryId == 1 || categoryId == 4) {
        ...
        toppingLabel.innerText = "Yêu cầu ghi chú món ăn:";
        toppingInput.placeholder = "Không hành tây, cay nhiều...";
    } else {
        drinkSection.style.display = 'block';
        toppingLabel.innerText = "Topping yêu cầu thêm:";
        toppingInput.placeholder = "Thêm trân châu...";
    }
...
function moGioHangHienTai() { if (globalCart.length === 0) alert("🛒 Giỏ hàng trống!"); else document.getElementById('cartModal').style.display = 'flex'; }
function dongGioHang() { document.getElementById('cartModal').style.display = 'none'; }
function validateCartBeforeSubmit() { if (globalCart.length === 0) { alert("⚠️ Giỏ hàng trống!"); return false; } return true; }
...
<?php if ($thong_bao == "thanh_cong"): ?>
    <script>
        alert('🎉 Đặt hàng thành công! Đơn hàng đã đồng bộ đồng loạt vào Database.');
        globalCart = [];
        capNhatGiaoDienGioHang();
    </script>
<?php elseif ($thong_bao == "vui_long_nhap_du"): ?>
    <script>alert('⚠️ Vui lòng nhập đầy đủ Tên, Số điện thoại và Địa chỉ nhận hàng!');</script>
<?php elseif ($thong_bao == "loi_chi_tiet_don" || $thong_bao == "loi_tao_don_hang"): ?>
    <script>alert('❌ Có lỗi hệ thống khi đồng bộ đơn hàng!');</script>
<?php endif; ?>
